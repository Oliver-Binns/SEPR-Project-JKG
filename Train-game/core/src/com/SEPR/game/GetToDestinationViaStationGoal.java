...
public class GetToDestinationViaStationGoal extends Goal {
	...
	@Override
	public boolean CheckComplete(ArrayList<Train> trainsList) { //CHECKS COMPLETE FOR A PARTICULAR PLAYER BY ITERATING
		int carriages = 0;
		for(int i = 0; i < trainsList.size(); i++){	//RUN THROUGH LIST OF TRAINS AND CHECK IF NO CARRIAGES IS SUFFICIENT...
			for(int j = 0; j < reachedStation.size(); j++){
				...
				if(trainsList.get(i).getTrainID() == pair[0]){
					carriages += pair[1];
				}
			}
		}
		return carriages >= this.getNumCarriages();
	}
